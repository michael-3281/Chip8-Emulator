...
public class Chip8
{
    ...
    private List<string> romList = new();
    ...
    // Menu list getter
    public List<string> GetRomList() => romList;
    ...
    public void StartRom(string path)
    {
        // Ensure the screen is cleared when leaving the launcher
        ClearDisplay();
    ...
    public void LoadRomList(string romFolder)
    {
        romList.Clear();
        ...
        romList = Directory.GetFiles(romFolder, "*.ch8").ToList();
        selectedRomIndex = 0;
        menuOffset = 0;
    }

    public void HandleMenuKey(ConsoleKey key)
    {
        if (mode != EmulatorMode.Launcher)
            return;
        
        if (key == ConsoleKey.W && selectedRomIndex > 0)
            selectedRomIndex--;
        
        if (key == ConsoleKey.S && selectedRomIndex < romList.Count - 1)
            selectedRomIndex++;
        
        if (key == ConsoleKey.Enter)
            StartRom(romList[selectedRomIndex]);
    }
    ...
    public void DrawLauncher()
    {
        ...
        for (int i = 0; i < visibleCount; i++)
        {
            ...
            string name = Path.GetFileNameWithoutExtension(romList[idx]);
